...
class HalcyonIHM:
    ...
    def _refresh_data(self):
        ...
        if data_active:
            ...
            self._lbl_temp4["val"].config(text=f"{t4:.1f}")
    ...
    def _save_graph(self):
        """
        Sauvegarde les courbes en PNG de façon thread-safe.
        N'utilise pas le canvas Tkinter (FigureCanvasTkAgg) pour éviter
        les problèmes de thread : crée une Figure matplotlib autonome.
        """
        ...
        # Création d'une figure indépendante (pas li�e � Tkinter)
        ...
        for i, (key, name) in enumerate(zip(self._temp_keys, self._temp_names)):
            ...
            if vals:
                ax_t.plot(
                    times_raw[-len(vals):], vals,
                    color=self._temp_colors[i],
                    linewidth=1.2,
                    label=name
                )
        ...
        if self.data.get("PUMP_ACTIVATION"):
            ...
            if valid_press:
                ...
                ax_p.plot(valid_times, valid_press, color=self._press_color, linewidth=1.0, linestyle="--", label="Pression")
            
            else:
                ax_p.plot([], [],color=self._press_color,linewidth=1.0,linestyle="--",label="Pression")   
        ...
        # libérer la mémoire 
        plt.close(fig)  
        print(f"[ihm] Graphique sauvegard� : {filename}")
